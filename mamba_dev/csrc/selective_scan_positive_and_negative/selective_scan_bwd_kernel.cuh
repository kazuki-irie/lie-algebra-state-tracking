    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            if constexpr (!kIsComplex) {
                #pragma unroll
                for (int i = 0; i < kNItems; ++i) {
                    const float delta_a_exp = 2.0f * exp2f(delta_vals[i] * A_scaled) - 1.0f;
                ...
                // );
                // Using Exclusive forward scan hoping that ti will shift the output by one to the right
                typename Ktraits::BlockScanT(smem_scan).ExclusiveScan(
                    thread_data, thread_data, SSMScanOp<weight_t>(), prefix_op
                );
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    const float delta_a_exp = 2.0f * exp2f(delta_vals[i] * A_scaled) - 1.0f;
                    const float ddelta_a_exp = delta_a_exp + 1;
                    // const float a = thread_data[i].y - (!kIsVariableB ? delta_vals[i] * float(u_vals[i]) : delta_vals[i] * float(u_vals[i]) * B_vals[i]);
                    const float a = ddelta_a_exp * thread_data[i].y; // now thread_data[i].y == h_{i-1}
                    const float hi = delta_a_exp * thread_data[i].y + (!kIsVariableB ? delta_vals[i] * float(u_vals[i]) : delta_vals[i] * float(u_vals[i]) * B_vals[i]); // gotta recompute the h_i state.
                    ddelta_vals[i] += ddelta_u * float(u_vals[i]) + dx * A_val * a;
                    dA_val += dx * delta_vals[i] * a;
                    // d/dA_val h_t = h_{t-1} ddelta_a_exp * delta_vals[i] * kLog2e + delta_a_exp * d/dA_val h_{t-1}
                    if constexpr (!kIsVariableB || !kIsVariableC) {
                        if constexpr (!kIsVariableB) {  // dBC_val is dB_val
                            dBC_val += dout_vals[i] * (!kIsVariableC ? hi : hi * C_vals[i]);
                        } else {  // dBC_val is dC_val
                            dBC_val += dout_vals[i] * hi;
                        }
                    }
                    if constexpr (kIsVariableB) { dB_vals[i] = dx * delta_vals[i] * float(u_vals[i]); }
                    if constexpr (kIsVariableC) {
                        dC_vals[i] = dout_vals[i] * (!kIsVariableB ? hi * B_val : hi);
                    }
                }
