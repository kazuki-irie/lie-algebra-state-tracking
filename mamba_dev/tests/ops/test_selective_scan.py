...
def test_selective_scan(is_variable_B=True, is_variable_C=True, varBC_groups=1, has_D=True, has_z=True,
                        has_delta_bias=True,
                        delta_softplus=True, return_last_state=True, seqlen=4, itype=torch.float32,
                        wtype=torch.float32):
    device = 'cuda'
    ...
    A = (-0.5 * torch.exp(torch.rand(dim, dstate, device=device, dtype=wtype))).requires_grad_()
    ...
    delta = (0.5 * torch.rand(batch_size, dim, seqlen, device=device, dtype=itype) + 10.0).requires_grad_()
    ...
    out, *rest = selective_scan_fn(
        u=u, delta=delta, A=A, B=B, C=C, D=D, z=z,
        delta_bias=delta_bias, delta_softplus=delta_softplus,
        return_last_state=return_last_state, positive_and_negative_associative_scan=False
    )
...
if __name__ == "__main__":
    for seqlen in [256]:
        ...
        test_selective_scan(seqlen=seqlen, itype=torch.float32, wtype=torch.float32)
    # test_mamba_inner_fn()
